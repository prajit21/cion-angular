...
export const basicProgress: progress[] = [
  {
    ...
    title:
      '  Progress components are built with two HTML elements, some CSS to set the width, and a few attributes.We use the <code>.progress </code> as a wrapper to indicate the max value of the progress bar. The <code>.progress-bar </code > requires an inline style, utility class, or custom CSS to set their width.',
    item: [
      {
        value: '0',
        percentage: '25%',
...
export const ProgressBarsStriped: progress[] = [
  {
    ...
    title:
      'Add <code>.progress-bar-striped</code> to any <code>.progress-bar </code> to apply a stripe via CSS gradient over the progress bars background color Using CSS Effects.',
    item: [
      {
        value: '10%',
