...
import { PRODUCTLIST } from '../../../shared/data/component/e-commerce/product-list';
...
function sort(support: productList[], column: SortColumn, direction: string): productList[] {
  if (direction === '' || column === '') {
    return support;
  } else {
...
function matches(ticket: productList, term: string, pipe: PipeTransform) {
  return (
    ticket.product_name.toLowerCase().includes(term.toLowerCase()) ||
...
export class ProductListService {
  ...
  private _support$ = new BehaviorSubject<productList[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let support = sort(PRODUCTLIST, sortColumn, sortDirection);
